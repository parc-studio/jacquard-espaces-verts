...
- If `frontend/` changed: run `pnpm check`
- If `studio/` changed: run `pnpm --filter studio exec tsc --noEmit`
- If both changed: run both checks
- If behavior-level changes were made: run `pnpm validate` when feasible
